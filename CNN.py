...
for index, file in enumerate(os.listdir(dataset_dir)):
    ...
    if index == 0:
        ...
        valence_label = valence_label[random_indices, :]
        train_label = valence_label[:int(0.6 * data.shape[0]), :]
        valid_label = valence_label[int(0.6 * data.shape[0]):int(0.8 * data.shape[0]), :]
        test_label = valence_label[:int(0.8 * data.shape[0]), :]
    else:
        data = file['data']
        random_indices = np.random.permutation(data.shape[0])
        data = data[random_indices, :, :, :]
        ...
        valence_label = one_hot(np.int32(file['valence_labels'][0]), 2)
        valence_label = valence_label[random_indices, :]
        train_label = np.concatenate([train_label, valence_label[:int(0.6 * data.shape[0]), :]])
        valid_label = np.concatenate([valid_label, valence_label[int(0.6 * data.shape[0]):int(0.8 * data.shape[0]), :]])
        test_label = np.concatenate([test_label, valence_label[int(0.8 * data.shape[0]):, :]])
...
for epoch in tqdm(range(epochs)):
    ...
    print('epoch: %d training loss: %.3f validation loss: %.3f' % (epoch + 1, train_loss/num_batches, valid_loss/num_batches))
